...
export default function Dashboard() {
  const [statsHariIni, setStatsHariIni] = useState({ totalOmzet: 0, totalSetoran: 0, totalHutang: 0 })
  ...
  const fetchData = async () => {
    ...
    if (switchMode === 'pedagang' && activeCabangId) {
      const [doRes, tsRes, hutangRes, pjRes, stokRes] = await Promise.all([
        ...
        supabase.from('hutang_pedagang').select('*, penjualan_harian(nomor_pj)').eq('cabang_id', activeCabangId).eq('status', 'belum_lunas'),
      ...
      setStatsHariIni({ totalOmzet, totalSetoran, totalHutang })
      setPenjualanHariIni(pjRes.data || [])
    }
  ...
  const ActionCard = ({ icon, title, subtitle, badge, onClick, color = '#aa7700', bg = '#fff8e0', border = '#ffe0a0' }) => (
    <div onClick={onClick}
    ...
    <Layout>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        {(aksiPedagang.doMasuk.length > 0 || aksiPedagang.tsPerluKirim.length > 0 || aksiPedagang.tsPerluTerima.length > 0 || aksiPedagang.hutangAktif.length > 0) && (
          <div style={{ marginBottom: '1.5rem' }}>
            <p style={{ fontSize: '13px', fontWeight: '500', color: '#888', margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Perlu tindakan</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {aksiPedagang.doMasuk.map(do_ => (
                <ActionCard key={do_.id}
                  icon="🚚" title={`DO ${do_.nomor_do} menunggu diterima`}
              ...
              {aksiPedagang.tsPerluTerima.map(ts => (
                <ActionCard key={ts.id}
                  icon="📥" title={`${ts.nomor_ts} menunggu diterima`}
              ...
              {aksiPedagang.hutangAktif.length > 0 && (
                <ActionCard
                  icon="💰" title={`${aksiPedagang.hutangAktif.length} hutang belum lunas`}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '1.5rem' }}>
          ...
          <div style={{ background: 'white', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem' }}>
            ...
            <p style={{ fontSize: '18px', fontWeight: '500', margin: 0 }}>{statsHariIni.totalStok || 0} cup</p>
          </div>
          <div style={{ background: 'white', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem' }}>
    ...
    <Layout>
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '10px', marginBottom: '1.5rem' }}>
          ...
          <div style={{ background: 'white', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem' }}>
            <p style={{ fontSize: '12px', color: '#888', margin: '0 0 4px' }}>Belum setor</p>
            <p style={{ fontSize: '18px', fontWeight: '500', margin: 0, color: (statsHariIni.totalOmzet - statsHariIni.totalSetoran) > 0 ? '#cc2222' : '#2d7a2d' }}>
              Rp {(statsHariIni.totalOmzet - statsHariIni.totalSetoran).toLocaleString('id-ID')}
            </p>
          </div>
          <div style={{ background: 'white', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {aksiDC.pbPending.map(pb => (
                <ActionCard key={pb.id}
                  icon="📋" title={`${pb.nomor_pb} — ${pb.cabang?.nama_cabang}`}
                  subtitle="Permintaan Barang menunggu approval"
                  onClick={() => navigate('/pb')} />
              ))}
              {aksiDC.tsPending.map(ts => (
                <ActionCard key={ts.id}
                  icon="🔄" title={`${ts.nomor_ts} — ${ts.cabang_asal?.kode_cabang} → ${ts.cabang_tujuan?.kode_cabang}`}
                  subtitle="Transfer Stok menunggu approval"
                  onClick={() => navigate('/ts')} />
              ))}
              {aksiDC.doDraft.map(do_ => (
                <ActionCard key={do_.id}
                  icon="🚚" title={`${do_.nomor_do} — ${do_.cabang?.nama_cabang}`}
        ...
        <div style={{ background: 'white', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem', marginBottom: '1rem' }}>
          ...
          {penjualanHariIni.length === 0 ? (
            ...
            <div key={pj.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < penjualanHariIni.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
              <div>
        ...
        {/* CABANG BELUM LAPOR */}
        <div style={{ background: 'white', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '1rem' }}>
          <p style={{ fontSize: '14px', fontWeight: '500', margin: '0 0 4px' }}>Ringkasan hari ini</p>
          <div style={{ display: 'flex', gap: '16px', fontSize: '13px', color: '#888', marginTop: '8px' }}>
            ...
            <span>📋 {penjualanHariIni.length} total laporan</span>
          </div>
        </div>
      </div>
    </Layout>
