...
export default function DeliveryOrder() {
  ...
  const fetchAll = async () => {
    setLoading(true)
    const [d, c, p, pb] = await Promise.all([
  ...
  const handleSubmit = async (kirimSekarang = false) => {
    ...
    const { data: newDO } = await supabase.from('delivery_order').insert(payload).select().single()

    if (newDO) {
      const produkData = produk
      await supabase.from('delivery_order_detail').insert(
        items.map(item => {
          const p = produkData.find(p => p.id === item.produk_id)
          return {
            do_id: newDO.id,
  ...
  const handleKirim = async (doItem) => {
    ...
    fetchAll()
    if (view === 'detail') fetchDetail({ ...doItem, status: 'kirim' })
  }
    ...
    <Layout>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ background: '#f9f9f9' }}>
                <th style={{ padding: '10px 16px', textAlign: 'left', fontWeight: '400', color: '#888', width: '45%' }}>Produk</th>
                <th style={{ padding: '10px 16px', textAlign: 'center', fontWeight: '400', color: '#888', width: '35%' }}>Qty kirim (cup)</th>
                <th style={{ padding: '10px 16px', textAlign: 'center', fontWeight: '400', color: '#888', width: '20%' }}></th>
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          <button onClick={() => handleSubmit(true)} disabled={saving} style={{ padding: '8px 16px', background: '#f0faf0', border: '1px solid #c0e0c0', borderRadius: '8px', fontSize: '13px', fontWeight: '500', color: '#2d7a2d', cursor: 'pointer' }}>
            ...
            <tbody>
              {detailData.detail?.map((item, i) => (
                <tr key={item.id} style={{ borderTop: '1px solid #f0f0f0', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                  ...
                  <td style={{ padding: '10px 16px', textAlign: 'center', color: item.qty_diterima < item.qty_kirim ? '#e05555' : '#2d7a2d' }}>
        ...
        {showCancel && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: 'white', borderRadius: '12px', padding: '1.5rem', width: '380px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                <button onClick={() => { setShowCancel(false); setAlasanCancel(''); setAlasanLain('') }} style={{ padding: '8px 16px', border: '1px solid #e0e0e0', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', background: 'white' }}>Batal</button>
                <button onClick={handleCancel} style={{ padding: '8px 16px', background: '#fff0f0', border: '1px solid #ffd0d0', borderRadius: '8px', fontSize: '13px', color: '#cc2222', cursor: 'pointer' }}>Konfirmasi cancel</button>
              </div>
            </div>
          </div>
        )}
